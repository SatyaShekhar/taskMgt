	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (oldProjectID == null || (!newProjectIdString.equals(oldProjectID.toString()))) {
		    ...
		    if (Action.Manage.name().equals(request.getParameter(PropertyNames.USER_ACTION))) {
                ...
                List<Iteration> iterations = sessionDB.createQuery("from Iteration where project.projectId = '" + newProjectIdString + "'").list();
		...
		logger.info("Result updated to session project id = " + session.getAttribute(PropertyNames.PROJECT_ID) + " Iteration = " + session.getAttribute(PropertyNames.ITERATION_ID));
